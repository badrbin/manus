...
def get_customer_gl_summary(customer=None):
    """
    تجميع بيانات العميل (فواتير، دفعات، رصيد)
    """
    if not customer:
    ...
    gl_aggregates = frappe.db.sql("""
        SELECT 
            SUM(CASE 
                WHEN voucher_type = 'Sales Invoice' THEN debit - credit 
                ELSE 0 
            END) as total_invoices,
            SUM(CASE 
                WHEN voucher_type != 'Sales Invoice' THEN credit - debit 
                ELSE 0 
...
def get_material_request_available_qty(material_request=None, current_doc=None):
    """
    جلب الكميات المتاحة من طلب المواد بعد خصم المستهلك سابقاً
    """
    ...
    for item in mr_items:
        qty_map[item.item_code] = qty_map.get(item.item_code, 0) + item.qty

    other_installations = frappe.get_all("Installation Note",
        filters={
    ...
    if other_installations:
        used_items = frappe.get_all("Installation Note Item",
            filters={"parent": ["in", other_installations]},
            fields=["item_code", "qty"]
        )
        for item in used_items:
            if item.item_code in qty_map:
                qty_map[item.item_code] = qty_map[item.item_code] - item.qty
...
def get_sales_order_summary(sales_order=None):
    """
    ملخص أمر البيع (المطلوب، المفوتر، المسلم، طلبات المواد، والتركيب)
    """
    if not sales_order:
    ...
    for item in so_items:
        if item.item_code in summary_data:
            ...
            summary_data[item.item_code] = {
                ...
                "billed_qty": item.billed_actual_qty, 
                "mr_qty": 0.0,
                "installed_qty": 0.0,
                "balance": item.qty,
                "is_extra": False
    ...
    inst_items = frappe.db.sql("""
        ...
        WHERE parent.docstatus = 1 AND parent.custom_sales_order = %s
    """, (sales_order), as_dict=1)

    for row in inst_items:
        if row.item_code in summary_data:
            summary_data[row.item_code]['installed_qty'] += row.qty
        else:
            summary_data[row.item_code] = {
                "item_code": row.item_code,
...
def get_sales_order_item_codes(sales_order=None):
    """
    جلب قائمة أكواد الأصناف في أمر البيع
    """
    if not sales_order:
        sales_order = frappe.form_dict.get('sales_order')
    if not sales_order:
...
def get_stock_entries_by_mr(mr_name=None):
    """
    جلب السندات المخزنية المرتبطة بطلب مواد
    """
    if not mr_name:
    ...
    entries = frappe.get_all("Stock Entry",
        filters={"custom_material_request": mr_name, "docstatus": ["<", 2]},
...
def get_supplier_gl_summary(supplier=None):
    """
    تجميع بيانات المورد (فواتير، دفعات، رصيد)
    """
    if not supplier:
